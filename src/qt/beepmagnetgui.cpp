// Copyright (c) 2014 The BeepMagnet developers
// Distributed under the MIT/X11 software license, see the accompanying
// file license.txt or http://www.opensource.org/licenses/mit-license.php.

#include "beepmagnetgui.h"
...
BeepMagnetGUI::BeepMagnetGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new BeepMagnetBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("BeepMagnet") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/beepmagnet"));
    setWindowIcon(QIcon(":icons/beepmagnet"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/beepmagnet/index.html");
#else
    QFile html("/opt/beepmagnet/index.html");
#endif
...
void BeepMagnetGUI::pageLoaded(bool ok)
{
...
void BeepMagnetGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void BeepMagnetGUI::urlClicked(const QUrl & link)
{
...
void BeepMagnetGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About BeepMagnet"), this);
    aboutAction->setToolTip(tr("Show information about BeepMagnet"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for BeepMagnet"));
...
void BeepMagnetGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void BeepMagnetGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("BeepMagnet") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/beepmagnet_testnet"));
            setWindowIcon(QIcon(":icons/beepmagnet_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/beepmagnet_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BeepMagnet client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/beepmagnet_testnet"));
...
void BeepMagnetGUI::setWalletModel(WalletModel *walletModel)
{
...
void BeepMagnetGUI::setMessageModel(MessageModel *messageModel)
{
...
void BeepMagnetGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("BeepMagnet client"));
    trayIcon->setIcon(QIcon(":/icons/beepmagnet"));
...
#ifndef Q_OS_MAC
void BeepMagnetGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BeepMagnetGUI::aboutClicked()
{
    AboutDialog dlg;
...
void BeepMagnetGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to BeepMagnet network", "", count));
}

void BeepMagnetGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void BeepMagnetGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void BeepMagnetGUI::changeEvent(QEvent *e)
{
...
void BeepMagnetGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BeepMagnetGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, nFeeRequired));
...
void BeepMagnetGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void BeepMagnetGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void BeepMagnetGUI::gotoSignMessageTab(QString addr)
{
...
void BeepMagnetGUI::gotoVerifyMessageTab(QString addr)
{
...
void BeepMagnetGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void BeepMagnetGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BeepMagnetGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void BeepMagnetGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BeepMagnet address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void BeepMagnetGUI::handleURI(QString strURI)
{
...
void BeepMagnetGUI::setEncryptionStatus(int status)
{
...
void BeepMagnetGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void BeepMagnetGUI::backupWallet()
{
...
void BeepMagnetGUI::changePassphrase()
{
...
void BeepMagnetGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void BeepMagnetGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BeepMagnetGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BeepMagnetGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BeepMagnetGUI::updateStakingIcon()
{
...
void BeepMagnetGUI::detectShutdown()
{
