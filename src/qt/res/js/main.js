...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            case 1:
                this.name = "mBMCoin",
                this.display = "mBMCoin";
                break;

            case 2:
                this.name = "uBMCoin",
                this.display = "&micro;BMCoin";
                break;

            case 3:
                this.name    = "sBMCoin",
                this.display = "sBMCoin";
                break;

            default:
                this.name = this.display = "BMCoin";
        }
...
var overviewPage = {
    ...
    updateBalance: function(balance, okcashBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            ...
            this.okcashBal  .data("orig", okcashBal),
        ...
        this.formatValue("okcashBal",   okcashBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. BAw1o5SYpuk1wQHLJcExMQFYzh4qb9k1GC)" placeholder="Enter a BeepMAGNET address (e.g. BAw1o5SYpuk1wQHLJcExMQFYzh4qb9k1GC)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="BMCoin"                    ' + (unit.type == 0 ? "selected" : "") + '>BMCoin</option> \
                    <option value="1" title="Milli-BMCoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mBMCoin</option> \
                    <option value="2" title="Micro-BMCoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;BMCoin</option> \
                    <option value="3" title="Nano-BMCoin (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>Nano-BeepMAGNET</option> \
                </select> \
            </div> \
