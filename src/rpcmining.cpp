...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "BeepMagnet is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "BeepMagnet is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BeepMagnet is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BeepMagnet is downloading blocks...");
...
Value submitblock(const Array& params, bool fHelp)
{
    ...
    if (fDebugNet)
        LogPrintf("\n\nNewly mined hashblock incomming! \n%s\n\n\n", hashblock.ToString().c_str());
    ...
    if (fDebugNet)
        LogPrintf("\n\nNewly mined hashblock accepted!!! \n%s\n\n\n", hashblock.ToString().c_str());
