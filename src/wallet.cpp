...
int64_t CWallet::GetBeepMagnetDebit(const CTxIn& txin) const
{
    if (!txin.IsAnonInput())
        return 0;

    // -- amount of owned beepmagnet decreased
...
int64_t CWallet::GetBeepMagnetCredit(const CTxOut& txout) const
{
...
int64_t CWallet::GetBeepMagnetBalance() const
{
    int64_t nTotal = 0;

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted() && pcoin->nVersion == ANON_TXN_VERSION)
                nTotal += pcoin->GetAvailableBeepMagnetCredit();
        };
    }
...
bool CWallet::SendOkToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (vNodes.empty())
    {
        sError = _("Error: BeepMagnet is not connected!");
        return false;
    };
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (nValue + nTransactionFee > GetBeepMagnetBalance())
    {
        sError = "Insufficient beepmagnet funds";
        return false;
    };
