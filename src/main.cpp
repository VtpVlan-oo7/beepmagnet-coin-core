...
unsigned int nModifierInterval  = 10 * 60; // time to elapse before new modifier is computed, 60 seconds
...
const string strMessageMagic = "BeepMagnet Signed Message:\n";
...
{
    AssertLockHeld(cs_main);
    LogPrintf("ProcessBlockThin: INIT\n\n");
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BeepMagnet", CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    };
    return true;
